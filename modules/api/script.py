...
def get_scripts_list():
    ...
    postproc = [script.name for script in scripts_manager.scripts_postproc.scripts if script.name is not None]
...
def init_default_script_args(script_runner):
    ...
    try:
        for script in script_runner.scripts:
            ui_default_values = script.default_args(script.is_img2img)
        ...
        log.error(f"Error initializing script args for {script.title()} {e}")
    return script_args
